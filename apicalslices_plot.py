...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 11 14:33:24 2021
...
#%%
im_test_ed_sub = np.concatenate((np.concatenate(data_im_ed_DCM).astype(None),
                                  np.concatenate(data_im_ed_HCM).astype(None),
                                  np.concatenate(data_im_ed_MINF).astype(None),
                                  np.concatenate(data_im_ed_NOR).astype(None),
                                  np.concatenate(data_im_ed_RV).astype(None)))


gt_test_ed_sub = np.concatenate((np.concatenate(data_gt_ed_DCM).astype(None),
...
for i in range(0,25):
    plt.subplot(5,5,i+1)
    plt.imshow(im_test_ed_sub[i + test_slice,0,:,:])
...
for i in range(0,8):
    plt.subplot(2,4,i+1)
    plt.imshow(im_test_ed_sub[i + 576,0,:,:])
    plt.title('Slice: {}'.format(i), fontsize =35)
...
for i in range(0,8):
    plt.subplot(2,4,i+1)
    plt.imshow(gt_test_ed_sub[i + 576,:,:])
    #plt.imshow(ref[i + test_slice,:,:,1])
    plt.title('slice: {}'.format(i), fontsize =25)
    plt.xticks(fontsize=15)
    plt.yticks(fontsize=15)
